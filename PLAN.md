...
- [x] **Replace xargs with sed for output trimming in boiler.sh**: In `boiler.sh`, several pipelines use `| tr -d '\n\r' | xargs` to trim values (e.g. in `sanitize_value`, `bps_to_mbps`, and downstream of `bc`). In some restricted environments, `xargs` fails with `sysconf(_SC_ARG_MAX) failed`, so Bats tests that source boiler.sh can fail there even though they pass in a normal terminal. **Condition:** Implement only if there is another way to get the same value (trimmed string) without breaking boiler's behavior. **Analysis:** There is: `sed 's/^[[:space:]]*//;s/[[:space:]]*$//'` provides the same trimming (leading/trailing whitespace), does not call `sysconf`, and is a drop-in replacement; remux-select-audio.sh already uses this sed pattern. Replacing each `| xargs` trimming step in boiler.sh with that sed preserves functionality and avoids the failure in restricted environments. **(Implemented:** Added `normalize_output()` helper doing tr+trim; all 12 call sites use it; helper uses sed instead of xargs so Bats runs in restricted environments where xargs can fail with sysconf(_SC_ARG_MAX) failed.)**
