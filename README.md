...
- **`./remux-select-audio.sh`** - Single-file remux with audio track selection. Pass one video file; the script lists audio tracks, lets you choose which to keep (e.g. drop commentary or extra languages), and remuxes to a Boiler-compatible file (stream copy, no transcoding). Output container matches input (e.g. MKV→`{base}.remux.mkv`, MP4→`{base}.remux.mp4`) so all subtitles are preserved. Output is in the same directory.
