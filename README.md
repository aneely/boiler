...
- **`./remux-select-audio.sh`** - Remux with audio track selection. Pass one or more video files or directories; the script lists audio tracks per file, lets you choose which to keep (e.g. drop commentary or extra languages), and remuxes to a Boiler-compatible file (stream copy, no transcoding). Can be run with no arguments (processes current directory, two levels deep by default) or with `-L`/`--max-depth` and path(s). Output container matches input (e.g. MKV→`{base}.remux.mkv`, MP4→`{base}.remux.mp4`) so all subtitles are preserved. Output is in the same directory.
